<template>
  <div>
    <el-dialog v-bind="$attrs" v-on="$listeners" :visible.sync="showDialog" @open="onOpen" @close="onClose" title="添加辅导员用户">
      <el-row :gutter="15">
        <el-form ref="elForm" :model="user" :rules="rules" size="medium" label-width="80px">
          <el-col :span="24">
            <el-form-item label="头像">
              <input type="file" ref="file2" name="check_img_url"
                     @change="tirggerFile($event)" aria-multiline="true" />
              <div>
...
<script>
  ...
  import { saveInstructors } from '../../api/instructors';
  ...
  export default {
    ...
    methods: {
      ...
      async addUser(){
        var that = this;
        const { data:res } = await saveInstructors(this.user,this.user.simg,this.imgtype,this.imgName);
        if(res.code == 200){
           this.$notify.success({
             title:'操作成功',
             message: '添加成功'
